...
export default function EditAppointmentPage() {
  ...
  const handleVehicleChange = (vehicleId: string) => {
    const actualVehicleId = vehicleId === "none" ? "" : vehicleId;
    setFormData(prev => ({ ...prev, vehicleId: actualVehicleId }));
    
    // Auto-select client if vehicle has an owner
    if (actualVehicleId) {
      const selectedVehicle = vehicles.find(v => v.id === actualVehicleId);
  ...
  return (
    ...
    <Select value={formData.clientId || "none"} onValueChange={(value) => {
      const actualClientId = value === "none" ? "" : value;
      setFormData(prev => ({ ...prev, clientId: actualClientId }));
    }}>
    <SelectTrigger>
